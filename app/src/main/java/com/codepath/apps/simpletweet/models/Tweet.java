 ...
 */
// Pass JSON + Store data, encapsulate login
public class Tweet implements Serializable {
    private String body;
    private long uid; // uniqueId for the tweet
    private User user;
    ...
    public static Tweet fromJSON(JSONObject jsonObject){
        ...
        // Extract value from JSON and store
        try {
            tweet.body = jsonObject.getString("text");
            tweet.uid = jsonObject.getLong("id");
            tweet.createdAt = getRelativeTime(jsonObject.getString("created_at"));
    ...
    // Convert Time String to relative time
    private static String getRelativeTime(String timeString){
        String fullRelativeTime = DateUtils.getRelativeTimeSpanString(getEpochTime(timeString), System.currentTimeMillis(), SECOND_IN_MILLIS).toString();
        String shortRelativeTime = fullRelativeTime
                .replace(" day", "d")
                .replace(" hour", "h")
                .replace(" minute", "m")
                .replace(" second", "x")
    ...
    // Convert Time String to timeInMillisSinceEpoch
    private static long getEpochTime(String timeString){
        /* "Tue Aug 28 21:16:23 +0000 2012" */
        SimpleDateFormat sdf  = new SimpleDateFormat("ccc LLL dd kk:mm:ss Z yyyy");
        try {
